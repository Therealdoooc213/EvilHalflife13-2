#define ROUND_END_ANNOUNCEMENT_TIME 150 //the time at which the game will announce that the shuttle can be called, in minutes.
...
/datum/controller/subsystem/Yogs/fire(resumed = 0)
	...
	if(world.time > (ROUND_END_ANNOUNCEMENT_TIME*600) && !endedshift && !(EMERGENCY_AT_LEAST_DOCKED))
		priority_announce("District, standard relocation time has been met.[SSshuttle.emergency.mode == SHUTTLE_IDLE ? "\nYou may call the train whenever you find it appropriate." : ""]", "End of cycle announcement", SSstation.announcer.get_rand_report_sound())
		endedshift = TRUE
