...
/obj/machinery/atm/attackby(obj/item/I, mob/user, params)
	...
	if(isidcard(I))
		...
		inserted_card = I
		updateUsrDialog()
		playsound(loc, 'sound/machines/atm/cardreader_insert.ogg', 50)
	...
	if(iscash(I))
		...
		updateUsrDialog()
		playsound(loc, 'sound/machines/atm/cash_insert.ogg', 50)
		return TRUE
...
/obj/machinery/atm/proc/dispense_cash(amt)
	...
	playsound(loc, 'sound/machines/atm/cash_desert.ogg', 50)
	reject_topic = TRUE
	sleep(1 SECONDS)
	reject_topic = FALSE
...
/obj/machinery/atm/Topic(href, href_list)
	...
	if(href_list["eject_id"])
		...
		inserted_card = null
		playsound(loc, 'sound/machines/atm/cardreader_desert.ogg', 50)
		updateUsrDialog()
		return TRUE
		...
		switch(key)
			if("E")
				if(try_authenticate(entered_pin))
					entered_pin = ""
					playsound(loc, 'sound/machines/atm/cardreader_read.ogg', 50)
					updateUsrDialog()
