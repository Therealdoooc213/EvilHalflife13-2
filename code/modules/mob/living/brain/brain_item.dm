...
/obj/item/organ/brain/Remove(mob/living/carbon/C, special = FALSE, no_id_transfer = FALSE)
	...
	if(!QDELETED(brainmob) && length(skillchips))
		if(!special)
			to_chat(brainmob, span_notice("You feel your skillchips enable emergency power saving mode, deactivating as your brain leaves your body..."))
