...
/datum/job/proc/get_exp_req_amount()
	if(title in (GLOB.command_positions | list("Dispatch")))
...
/proc/job_is_xp_locked(jobtitle)
	if(!CONFIG_GET(flag/use_exp_restrictions_heads) && (jobtitle in (GLOB.command_positions | list("Dispatch"))))
		return FALSE
	if(!CONFIG_GET(flag/use_exp_restrictions_other) && !(jobtitle in (GLOB.command_positions | list("Dispatch"))))
		return FALSE
	return TRUE
...
/client/proc/update_exp_list(minutes, announce_changes = FALSE)
	...
	//This doesn't use is_mentor(), thats intentional, we don't want admins to rack up mentor playtime because theyre admins not mentors
	if(mentor_datum)
		play_records[EXP_TYPE_MENTOR] += minutes
		if(announce_changes)
			to_chat(src,span_notice("You got: [minutes] Mentor EXP!"))
